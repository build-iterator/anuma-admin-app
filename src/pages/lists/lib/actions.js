...
// `mutate` is awaited by every caller below (and by bulk actions fanning out
// over many records) so a failed PATCH surfaces as a rejection instead of
// silently vanishing. The event-log write is best-effort and never blocks
// or fails the caller on its own account.
async function mutate(list, record, patch, eventText) {
  await updateRecord(list.id, record.id, patch);
  if (eventText) logEvent(list.id, record.id, eventText).catch(() => {});
}

export function setStage(list, record, stage) {
  const from = record.values[list.stageField];
  if (stage === from) return Promise.resolve();
  return mutate(list, record, { [list.stageField]: stage }, `Stage ${from ?? "—"} → ${stage}`);
}

export function setOwner(list, record, owner) {
  if (owner === (record.values.owner ?? "")) return Promise.resolve();
  return mutate(list, record, { owner: owner || undefined }, owner ? `Assigned to ${owner}` : "Unassigned");
}

export function setNextAction(list, record, action, date) {
  const cur = record.values;
  if (action === (cur.next_action ?? "") && date === (cur.next_action_date ?? "")) return Promise.resolve();
  return mutate(
    list,
    record,
...
export function setTemperature(list, record, key) {
  if (key && !TEMPS[key]) return Promise.resolve();
  return mutate(
    list,
    record,
...
export function snooze(list, record, days = FOLLOW_UP_DAYS) {
  ...
  return mutate(list, record, { next_action_date: date }, `Snoozed to ${date}`);
}
